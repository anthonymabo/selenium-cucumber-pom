    ...
    stages {
        stage('Export features') {
                    steps {
                        ...
                        bat 'curl -H "Content-Type: application/json" -X GET -H "Authorization: Bearer ${TOKEN}" "https://xray.cloud.getxray.app/api/v2/export/cucumber?keys=${params.TEST_PLAN_KEY}" --output features.zip'
